    ...
    def __init__(self, raw_buff):
        ...
        if axml_file != CHUNK_AXML_FILE:
            ...
            if axml_file >> 16 == 0x0008:
                ...
                log.warning("AXML file has an unusual header. Header: 0x{:08x}".format(axml_file))
            else:
            ...
            while nb < len(self.packages[package_name]):
                ...
                if isinstance(header, ARSCHeader):
                    if header.type == RES_TABLE_TYPE_TYPE:
                        ...
                        language = toUnicode(a_res_type.config.get_language())
                        region = toUnicode(a_res_type.config.get_country())
                        if region == "\x00\x00":
                            locale = language
                        else:
